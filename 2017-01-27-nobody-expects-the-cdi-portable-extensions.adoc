...
<T extends com.codahale.metrics.Metric> void processMetricProducer(@Observes ProcessProducer<?, T> pp, BeanManager bm) { <1>
        ...
        if (m != null) { <3>
            ...
            pp.configureProducer() <5>
                    .produceWith(ctx -> { <6>
                        MetricRegistry reg = bm.createInstance().select(MetricRegistry.class).get(); <7>
                        if (!reg.getMetrics().containsKey(name)) <8>
                            reg.register(name, pp.getProducer().produce(ctx)); <9>
                        return (T) reg.getMetrics().get(name);  <10>
                    });
        }
    }
}
...
<5> we use the new `ProducerConfigurator` helpers. In CDI 1.2 we would have created our own implementation of `Producer` interface
<6> we define a functional callback for producing the instance of the producer
<7> retrieving the registry bean instance
<8> looking for a metric with the matching name
<9> if it doesn't exist we create it by sing the original producer code and it to the registry
<10> we return the metric with the matching name from the registry
